...
UA = "AI-Intel-Radar/1.3 (free public-feed dashboard; contact via GitHub repository)"
...
def reddit_collect(config):
    ...
    for community in settings.get("communities", []):
        ...
        for url in urls:
            try:
                feed = request_feed(url, user_agent="AIIntelRadar/1.3 by u/lanjy7686-dot (public RSS reader)")
                if feed.entries:
                    break
            except Exception:
                feed = None
        ...
        for entry in feed.entries[:per_community]:
            ...
            articles.append({"title": title, "url": url, "source": f"Reddit · r/{community}", "source_type": "Reddit", "published_at": dt(entry.get("published") or entry.get("updated")).isoformat(), "author": author, "content": summary, "reddit_community": community})
        time.sleep(0.8)
    return articles
...
def wechat_collect(config):
    path = ROOT / config.get("wechat_urls_file", "config/wechat_urls.txt")
    if not path.exists():
        return []
    articles = []
    ...
    for url in urls:
        try:
            ...
            if title:
                articles.append({"title": title, "url": url, "source": author or "微信公众号", "source_type": "微信公众号", "published_at": datetime.now(timezone.utc).isoformat(), "author": author, "content": body})
        except Exception:
            continue
    return articles
...
def main():
    ...
    collectors = [("RSS", rss_collect), ("Reddit", reddit_collect), ("arXiv", arxiv_collect), ("Hacker News", hackernews_collect), ("微信", wechat_collect)]
    if config.get("gdelt", {}).get("enabled", False):
        collectors.append(("GDELT", gdelt_collect))
    all_items, errors = [], []
    source_status = {"X": {"enabled": False, "state": "free_search_only", "count": 0, "message": "完全免费模式：不调用X API，网页提供一键X实时搜索。"}}
    ...
    print(json.dumps({"count": len(output), "errors": errors, "reddit": source_status.get("Reddit")}, ensure_ascii=False))
